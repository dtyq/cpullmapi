...
func (s *Qwen3ASRSession) FeedAudioSamples(samples []float32, rTrimTokens uint) (lang string, text string, err error) {
	var outputTokens *C.llama_token = nil
	var nOutputTokens C.size_t = 0
	...
	// get length
	nTokens := C.llama_detokenize(s.vocab, outputTokens, C.int32_t(nOutputTokens), nil, 0, false, false)
	if nTokens == 0 {
		// no tokens generated
		return "", "", nil
	}
	textBuf := make([]byte, -nTokens)
	C.llama_detokenize(s.vocab, outputTokens, C.int32_t(nOutputTokens), (*C.char)(unsafe.Pointer(&textBuf[0])), C.int32_t(len(textBuf)), false, false)
